...
# Installs latest Node.js LTS from official nodejs.org releases for Nuxt.js development.
# - Detects OS + CPU arch
...
if need_cmd node; then
  ...
  if [[ "$major" -ge 20 ]]; then
    echo "Node.js for Nuxt.js is already installed ($ver). No action needed."
    npm -v || true
    exit 0
  fi
  echo "An older version of Node.js was detected ($ver); proceeding to install the required LTS version for Nuxt.js."
fi
...
echo "Downloading Node.js LTS for Nuxt.js: $URL"
...
echo "Installing Node.js for Nuxt.js to $INSTALL_DIR"
rm -rf "$INSTALL_DIR"
mkdir -p "$INSTALL_DIR"
...
echo ""
echo "Successfully installed Node.js ${VERSION} for Nuxt.js under $INSTALL_BASE/current"
echo "To complete the Nuxt.js setup, please restart your shell/terminal, then verify the installation:"
echo "  node -v"
echo "  npm -v"